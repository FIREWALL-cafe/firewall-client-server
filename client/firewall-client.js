...
function checkPendingQuery() {
	...
	if (pendingQuery &&
		...
		console.log('[checkPendingQuery] Found a pending query from', pendingQuery.source, ':', pendingQuery.query);
...
function checkURLQuery() {
    ...
    if(query) console.log("[checkURLQuery] overwriting", query, "with", queryMatch)
		query = queryMatch;
		ignoreSubmit = false;
		// toggleInputField(true, function(){
		// 	console.log('Input field enabled.');
    ...
    if(!pendingQuery) {
        ...
        console.log('[checkURLQuery] creating new pendingQuery object');
				...
				storage.set({
					...
					console.log('[checkURLQuery] Saved query to pending: ' + result.query);
				});
				startGettingImages();
			});
    } else {
...
function getImages() {
	...
	if (getSource() === 'google') {
		getGoogleDatastore().catch((error) => { console.error(error)
		}).then((data) => {
	...
	if (images.length < numImages &&
		...
		console.log('[getImages] Still only have ' + images.length + ' images; retry in 2 seconds (' + pendingQuery[retryKey] + ' of ' + maxRetries + ')');
		startGettingImages();
...
function checkPendingImages() {
	if (pendingQuery && pendingQuery.googleImages && pendingQuery.baiduImages) {
		...
		if (pendingQuery.googleImages.length) {
			console.log('[checkPendingImages] Looks like we have', pendingQuery.googleImages.length, 'images from Google!');
		} else {
			console.log('[checkPendingImages] No image results from Google. :(');
		}

		if (pendingQuery.baiduImages.length) {
			console.log('[checkPendingImages] Looks like we have', pendingQuery.baiduImages.length, 'images from Baidu!');
		} else {
